...
export default class FeedRoutes {
  public routes(app) {
    ...
    app.get('/feed/amoeba', async (req: Request, res: Response) => {
      try {
        const listings = await FeedItem.find({type: 'amoeba'});
        res.json({ listings });
      } catch(err) {
        res.status(400).json({message: 'Something went Wrong'})
        console.error(err);
    ...
    app.get('/feed/ttl', async (req: Request, res: Response) => {
      try {
        const listings = await FeedItem.find({type: 'ttl-exclusive'});
        res.json({ listings });
      } catch(err) {
        res.status(400).json({message: 'Something went Wrong'})
        console.error(err);
